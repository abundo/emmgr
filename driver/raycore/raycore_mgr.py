...
class IOS_Manager(emmgr.lib.basedriver.BaseDriver):
    ...
    def connect(self):
        ...
        while True:
            match = self.em.expect( { "username": r"Username:", "password": r"assword:" } )
            if match == "username":
                # self.em.writeln(self.username + )
                self.em.write(self.username + "\r\n")
                continue
            elif match == "password":
                # self.em.writeln(self.password)
                self.em.write(self.password + "\r\n")
                break
        ...
        #     self.em.write("enable\r\n")
        #     match = self.em.expect(r"assword:")
        #     if match is None:
        #         raise self.ElementException("Error waiting for enable password prompt")
        #     self.em.write(self.password + "\r\n")
        #     #self.em.writeln(self.password)
...
#            self.save_running_config()
    ...
    def save_running_config(self, callback=None):
        ...
        Raycore always commits directly to startup-config, this is an no-op
        """
        if callback:
