...
class IOS_Manager(BaseDriver):
    ...
    def connect(self):
        ...
        if self.use_ssh:
            ...
            self.transport = comm.RemoteConnection(timeout=20, method="ssh", codec='ISO-8859-1')
        else:
            if self.port == None: self.port = 23
            self.transport = comm.RemoteConnection(timeout=20, method="telnet", codec='ISO-8859-1')
